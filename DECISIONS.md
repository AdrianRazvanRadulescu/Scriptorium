...
23. **Font applied inside CodeMirror theme, not only via external CSS** — CodeMirror 6's base theme ships with `font-family: monospace` and uses a generated class selector (`.cm-editor.cm-HASH`) with higher specificity than a plain `.cm-editor` rule. To override it cleanly, `fontFamily`, `fontSize`, and `lineHeight` are set directly inside `buildEditorTheme()`'s `'&'` rule. These CSS variable references are resolved dynamically so theme and font settings still update live.

24. **10 built-in themes, two rows in Settings** — the default 6 themes (nocturne, ember, tidewater, vellum, foolscap, terminal) were extended with 4 literary-focused additions: Sepia (warm parchment), Midnight (deep navy), Dusk (purple-gray atmospheric), Manuscript (stark white). THEME_ORDER controls display order in the settings swatch grid. Adding more themes requires only a new entry in `themes.ts` and adding its id to `THEME_ORDER`.
